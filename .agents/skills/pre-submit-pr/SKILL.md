...
2. **Run all automated hooks**:
   - `bash .Codex/hooks/lint.sh` - format check (includes ruff format + ruff check)
   - `bash .Codex/hooks/test.sh` - run tests
   - `bash .Codex/hooks/check-debug.sh` - find debug code

3. **Run alignment review**:
   - Read `.Codex/docs/PRINCIPLES.md` and `.Codex/docs/INVARIANTS.md`
...
5. **Documentation freshness check**:
   - Review `.Codex/docs/REPO_WALKTHROUGH.md` against the current repo structure
   ...
   - Check triggers: new directories in `src/`, `envs/`, `.Codex/`, or `rfcs/`
...
7. **After push/PR creation**, run the post-push check:
   - `bash .Codex/hooks/post-push-pr.sh`
