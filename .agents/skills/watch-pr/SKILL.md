...
```bash
bash .Codex/hooks/ci-wait.sh <PR_NUMBER> 1800
```

**Important**: Set the Bash tool timeout to 600000ms (10 minutes). If the script exceeds
this, re-invoke it with the remaining timeout: `bash .Codex/hooks/ci-wait.sh <PR_NUMBER> <REMAINING_SECONDS>`.
...
5. Verify locally:
   ```bash
   bash .Codex/hooks/lint.sh && bash .Codex/hooks/test.sh
   ```
...
4. If it conflicts with `.Codex/docs/PRINCIPLES.md` or `.Codex/docs/INVARIANTS.md`: do NOT apply it. Reply explaining why:
   ```bash
